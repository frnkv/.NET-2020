...
namespace Lab04
{
    class ContactosMySQL : Contactos
    {
        protected string connectionString
        {
            get
            {
                return "server=DESKTOP-PUTRC38\\SQLEXPRESS;database=net;";
            }
        }
        ...
        public override DataTable getTabla()
        {
            ...
            {
                MySqlCommand command = new MySqlCommand("select * from contactos", conn);
        ...
        public override void aplicaCambios()
        {
            using (MySqlConnection Conn = new MySqlConnection(connectionString))
            {
               ...
               "values(@id,@nombre,@apellido,@email,@telefono)", Conn);
                ...
                DataTable filasNuevas = this._misContactos.GetChanges(DataRowState.Added);
                DataTable filasBorradas = this._misContactos.GetChanges(DataRowState.Deleted);
                DataTable filasModificadas = this._misContactos.GetChanges(DataRowState.Modified);
                ...
                Conn.Close();
                this._misContactos.AcceptChanges();
            }
        }
